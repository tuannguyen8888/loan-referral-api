...
import {GetLoanProfilesRequestDto, LoanProfileDto, LoanProfilesResponseDto} from "./dto";
...
export class LoanProfileService extends BaseService  {
    ...
    private convertEntity2Dto(entity) {
        ...
        for (let dtoKey of dtoKeys) {
            for (let entityKey of entityKeys) {
                if (dtoKey.toLowerCase().split('_').join('') == entityKey.toLowerCase()) {
    ...
    {
        const loanProfile = await this.connection.getCustomRepository(LoanProfileRepository).findOneOrFail(loanProfileId);
        if(loanProfile){
            let result = this.convertEntity2Dto( loanProfile);
            return result;
        }else{
            throw new BadRequestException([`loan_profile_id ${loanProfileId} is not exits.`])
        }
    }
    ...
    async createLoanProfile(dto: LoanProfileDto) {
        ...
        let member = await this.connection.getCustomRepository(LoanProfileRepository).save(entity);
