...
import {
    ...
    UpdateDeferRequestDto, DeferResponseDto
} from "./dto";
...
export class PtfLoanProfileService extends BaseService {
    ...
    async getLoanProfile(loanProfileId: number) {
        ...
        response.documentPhoto = this.convertEntities2Dtos(
            attachFiles,
            PtfAttachFile,
            AttachFileDto
        );
        ...
        response.process = this.convertEntities2Dtos(process, Process, ProcessDto);
        response.defers = this.convertEntities2Dtos(defers, PtfLoanProfileDefer, DeferResponseDto);
        return response;
    }
    ...
    async createLoanProfile(dto: LoanProfileDto) {
        ...
        let currentAddress: PtfAddress = this.convertDto2Entity(
            dto.currentAddress,
            PtfAddress
        );
        currentAddress.loanProfileId = result.id;
        currentAddress.addressType = "CURRES";
        let permanentAddress: PtfAddress = this.convertDto2Entity(
        ...
        let employmentInformation: PtfEmploymentInformation = this.convertDto2Entity(
            dto.employmentInformation,
            PtfEmploymentInformation
        );
        ...
        let relatedPersons: PtfRelatedPerson[] = this.convertDtos2Entities(
            dto.relatedPersons,
            PtfRelatedPerson
        );
    ...
    async updateAttachFiles(dtos: AttachFileDto[]) {
        await this.connection
            ...
            .execute();
        let entities: PtfAttachFile[] = this.convertDtos2Entities(
            dtos,
            PtfAttachFile
        );
    ...
    async updateLoanProfile(dto: LoanProfileUpdateDto) {
        let entityUpdate: PtfLoanProfile = this.convertDto2Entity(
            dto,
            PtfLoanProfile
        );
        ...
        let result = await this.connection
            .getCustomRepository(PtfLoanProfileRepository)
            .save(entityUpdate);
        let address = this.convertDtos2Entities(
            [dto.currentAddress, dto.permanentAddress],
            PtfAddress
        );
        ...
        let employmentInformation = this.convertDto2Entity(
            dto.employmentInformation,
            PtfEmploymentInformation
        );
        employmentInformation = await this.connection
        ...
        let relatedPersons = this.convertDtos2Entities(
            dto.relatedPersons,
            PtfRelatedPerson
        );
            ...
            if (employmentInformationEntity) {
                employmentInformation = {
                    address: {
                        ...
                        employmentInformationEntity.houseNumberAndStreet,
                    ...
                    employmentInformationEntity.companyUniversityName,
