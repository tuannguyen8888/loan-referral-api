...
import {PtfMasterDataRepository, PtfProductRepository} from "../../../repositories";
...
export class PtfMasterDataService extends BaseService {
    ...
    async getMasterDatas(type: string, parent1_key: string = null, parent2_key: string = null, keyWord: string) {
        try {
    ...
    async getBankBranches(parent1_key: string = null, parent2_key: string = null, keyWord: string) {
        try {
            const repo = this.connection.getCustomRepository(PtfMasterDataRepository);
            const where: any = {
                //deletedAt: IsNull(),
            ...
            const data = await repo.find(options);
            const result: BankBranchDto[] = this.convertMasterDatas2Dtos(data, BankBranchDto);
            return result;
        } catch (e) {
            console.error(e);
            throw e;
    ...
    async getLoanStatus(keyWord: string) {
        try {
            ...
            const data = await repo.find(options);
            const result: LoanStatusDto[] = this.convertMasterDatas2Dtos(data, LoanStatusDto);
            return result;
        } catch (e) {
            console.error(e);
            throw e;
    ...
    private convertMasterDatas2Dtos(entities, dtoClass = MasterDataDto) {
        ...
        if (entities && entities.length) {
            entities.forEach(entity =>
                dtos.push(this.convertMasterData2Dto(entity))
            );
        }
        return dtos;
    }
