...
export class McAttachfileController {
    ...
    getAttachfile(@Headers() headers, @Param() params): Promise<McAttachfileResponseDto> {
        return this.service.getAttachfile(params.id);
    }

    @Post("/")
    @ApiOperation({summary: "Thêm Attachfile"})
    createAttachfile(@Headers() headers, @Body() dto: McAttachfileDto): Promise<McAttachfileDto> {
        return this.service.createAttachfile(dto);
    }

    @Put("/")
    @ApiOperation({summary: "Sửa Attachfile"})
    updateAttachfile(@Headers() headers, @Body() dto: McAttachfileUpdateDto): Promise<McAttachfileUpdateDto> {
