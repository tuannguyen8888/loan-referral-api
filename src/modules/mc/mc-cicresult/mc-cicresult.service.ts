...
@Injectable()
export class McCicresultService extends BaseService{
    constructor(
    ...
    async getAllCicResults(dto: GetMCCicResultRequestDto) {
        try {
            ...
            query = query
                ...
                .take(dto.pagesize);
            const result = new McCicresultsResponseDto();
    ...
    async getCicResult(id: number) {
        let result = await this.connection
            .getCustomRepository(McCicresultRepository)
            .findOne(id);
        let response: McCicresultResponseDto = this.convertEntity2Dto(
            result,
            McCicResult,
            McCicresultResponseDto
        );
    ...
    async updateCicResult(dto: McCicresultUpdateDto) {
        let entityUpdate: McCicResult = this.convertDto2Entity(
            dto,
            McCicResult
        );
        ...
        let result = await this.connection
            .getCustomRepository(McCicresultRepository)
            .save(entityUpdate);
        let response: McCicresultUpdateDto = this.convertEntity2Dto(
            result,
            McCicResult,
            McCicresultUpdateDto
        );
        return response;
    }
