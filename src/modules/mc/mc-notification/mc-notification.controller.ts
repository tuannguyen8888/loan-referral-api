...
export class McNotificationController {
    ...
    getNotification(@Headers() headers, @Param() params): Promise<McNotificationResponseDto> {
        return this.service.getNotification(params.id);
    }

    @Post("/")
    @ApiOperation({summary: "Thêm Notification"})
    createNotification(@Headers() headers, @Body() dto: McNotificationDto): Promise<McNotificationDto> {
        return this.service.createNotification(dto);
    }

    @Put("/")
    @ApiOperation({summary: "Sửa Notification"})
    updateNotification(@Headers() headers, @Body() dto: McNotificationUpdateDto): Promise<McNotificationUpdateDto> {
