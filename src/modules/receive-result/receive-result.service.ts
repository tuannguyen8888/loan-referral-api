...
import {LoanProfileDeferRepository, LoanProfileRepository, ProcessRepository} from "../../repositories";
import {UploadDeferRequestDto, UploadDeferReponseDto, UploadStatusF1RequestDto, UploadStatusF1ReponseDto} from "./dto";
...
export class ReceiveResultService extends BaseService {
    ...
    async receiveDefer(dto: UploadDeferRequestDto) {
        ...
        if(loanProfile) {
            ...
            newDefer = await this.connection.getCustomRepository(LoanProfileDeferRepository).save(newDefer);
            ...
            await this.connection.getCustomRepository(ProcessRepository).save(newProcess);
        ...
        if(dtos && dtos.length){
            dtos.forEach( async dto=>{
                ...
                if(loanProfile){
                    ...
                    newProcess.description = dto.status_f1 + ((dto.reason || dto.rejected_code) ? (' ' + dto.reason + '[' + dto.rejected_code + ']' ): '');
